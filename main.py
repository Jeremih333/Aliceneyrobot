...
def format_actions(text: str) -> str:
    # Ищем паттерны действий и форматируем их
    return re.sub(r'\*(.*?)\*', r'`*\1*`', text)

# Функция для добавления эмодзи в конце предложений
def add_emojis(text: str) -> str:
    # Разделяем текст на предложения
    sentences = re.split(r'(?<=[.!?])\s+', text)
    
    # Добавляем эмодзи к каждому предложению
    result = []
    for sentence in sentences:
        if sentence.strip():
            # Добавляем 1-2 случайных эмодзи
            emoji_count = random.randint(1, 2)
            selected_emojis = ''.join(random.choices(EMOJI_LIST, k=emoji_count))
            result.append(f"{sentence}{selected_emojis}")
...
def clean_response(response: str) -> str:
    ...
    # Добавляем эмодзи в конце предложений
